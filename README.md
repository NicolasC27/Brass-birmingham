...
Members read each other in their own tongue: a post written in one of the four languages is rendered into the reader's by Claude (`app/server/translate.ts`), once per post and language, and kept; the reader sees "translated from French" under it and can show the original. It takes `ANTHROPIC_API_KEY` on the server — in the shell, or in `.env.local` at the root or beside the app, which the server reads at start and git ignores; without it everyone reads originals. `TRANSLATE_BUDGET_USD` caps what the interpreter may spend, all time (10 dollars by default, counted from the tokens each answer reports; the moderators' page shows the running total), and `TRANSLATE_MODEL` picks the model (`claude-opus-5` by default).
