...
describe('an analysis kept', () => {
  ...
  it('comes back as it was written, to the point it is shown at', () => {
    ...
    expect(back!.deep[0].chance).toBeCloseTo(0.5123, 6);
    expect(back!.deep[0].passes).toBe(3);
    expect(back!.verdicts[0].grade).toBe('mistake');
    expect(back!.roads['0|'][0].chance).toBeCloseTo(0.3333, 6);
    /* the table, the deal and the seat each name their own entry */
    expect(readKept(analysisKey('abcd', 7, 1), 30)).toBeNull();
  ...
  it('keeps a handful of games, the least recently read going first', () => {
    for (let g = 0; g < 12; g++) keepAnalysis(analysisKey(`t${g}`, 1, 0), 30, kept());
    expect(shelf.length).toBe(8);
    /* the last games written are the ones still there */
    expect(readKept(analysisKey('t11', 1, 0), 30)).not.toBeNull();
    expect(readKept(analysisKey('t0', 1, 0), 30)).toBeNull();
  });
});
