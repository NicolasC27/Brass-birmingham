...
import { provenance, saleBeer, slotAt } from '../provenance';
import type { PlanPicks } from '../provenance';
...
describe('where a railway burns its coal', () => {
  it('points the mine at the middle of the rail being laid', () => {
    const { s, me } = table();
    s.era = 'rail';
    s.links = {};
    const def = LINKS.find((l) => l.id === 'wolverhampton--coalbrookdale')!;
    const t = linkTargets(s, me).find((x) => x.valid && x.link.id === def.id)!;
    expect(t).toBeDefined();
    const p = provenance(s, me, picks({ verb: 'network', linkPick: t }), none);
    expect(p.threads).toHaveLength(1);
    expect(p.threads[0]).toMatchObject({ resource: 'coal', amount: 1, key: 'wolverhampton:1', onTile: false });
    expect(p.threads[0].to).toEqual(routeFor(def, 'rail').mid);
  });
});
