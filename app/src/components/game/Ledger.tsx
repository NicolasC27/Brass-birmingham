...
import { CircleMinus, Coins, Compass, FlaskConical, Hammer, Info, Landmark, Trophy, Waypoints } from 'lucide-react';
...
export default function Ledger({ seen = 0 }: { seen?: number }) {
  ...
  return (
    <section aria-label={t('game.ledger.aria')} className="plate relative flex h-full min-h-0 flex-1 flex-col overflow-hidden p-3">
      ...
      <ol ref={listRef} aria-live="polite" className="relative min-h-0 flex-1 overflow-y-auto pr-1 text-[12px]">
        <AnimatePresence initial={false}>
          {visible.map((e, idx) => {
            ...
            const named = e.player !== undefined && (sep || !prev || prev.player !== e.player || e.id === firstNew);
            ...
            return (
              <motion.li
                ...
                animate={{ opacity: fresh ? 1 : age > 30 ? 0.6 : age > 12 ? 0.8 : 1, y: 0 }}
                exit={{ opacity: 0 }}
              ...
              >
                ...
                {named && (
                  <p className={cn('mb-0.5 px-1 font-sans text-[11px] font-bold tracking-wide', !sep && prev && 'mt-2')} style={{ color }}>
                ...
                <div className={cn('flex items-start gap-1', fresh && 'border-l-2 border-brass-400/80')}>
                  <button
                    ...
                    className={cn(
                      'flex min-w-0 flex-1 items-start gap-2 rounded-sm px-1 py-1 text-left transition-colors hover:bg-brass-500/10',
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      {detail && <span className="mt-0.5 block font-sans text-[10.5px] leading-snug text-cream-100/50">{detail}</span>}
                    </span>
                  </button>
