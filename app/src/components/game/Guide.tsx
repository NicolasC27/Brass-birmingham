...
import { barrelBonuses, closingWords, dryRound, firstPayday, forgeWays, forgesFromMines, loanWords, stepKeyOf } from './lessonWords';
...
function Guide({ dock = 0 }: { dock?: number }) {
  ...
  const dueId = review ? review.id : (owed?.id ?? null);
  ...
  const behind = showSteps ? readBack(settled, review) : null;
