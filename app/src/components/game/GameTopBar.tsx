...
function useBand(
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const left =
        rail && !narrow
          ? Math.round(rail.getBoundingClientRect().right) + 12
          : 12;
      const right = market
        ? Math.round(window.innerWidth - market.getBoundingClientRect().left) +
          12
        : 12;
      setBand((prev) =>
...
export default function GameTopBar({
  ...
  const card = selectedCardId
    ? p.hand.find((c) => c.id === selectedCardId)
    : undefined;
  const summary = mine
    ? confirmSummary({
        verb,
  ...
  const cost = summary
    ? confirmCost(
        { verb, buildPick, linkPick, secondLinkPick, developPick },
        game,
      )
    : null;
  ...
  return (
    <div
      className="pointer-events-none fixed top-[44px] z-[64] flex justify-center"
      style={{ left: band.left, right: band.right }}
    >
      <div className="pointer-events-auto relative flex max-w-full flex-col rounded-lg border border-brass-700/60 bg-coal-900/90 shadow-e3 backdrop-blur-md">
        <div
          aria-hidden
          className="tex-paper pointer-events-none absolute inset-0 rounded-lg opacity-[0.05]"
        />
        <div className="flex h-[52px] items-stretch">
          ...
          <Tooltip
            ...
            title={t("game.topbar.eraRoundTitle", {
              era:
                game.era === "canal"
                  ? t("game.topbar.eraCanal")
                  : t("game.topbar.eraRail"),
              round: game.round,
              total,
            })}
            content={
              game.era === "canal"
                ? t("game.topbar.canalTip")
                : t("game.topbar.railTip")
            }
          >
            <span className="relative flex h-full flex-col items-center justify-center whitespace-nowrap border-r border-brass-700/40 px-3 leading-none">
              <span
                className={cn(
                  "font-fell text-[12px] tracking-[0.18em]",
                  game.era === "canal"
                    ? "text-cream-100"
                    : "text-copper-500 brightness-150",
                )}
                style={{
                  textShadow:
                    "0 1px 0 rgba(0,0,0,.8), 0 0 8px rgba(201,164,92,.25)",
                }}
              >
                {game.era === "canal"
                  ? t("game.topbar.badgeCanal")
                  : t("game.topbar.badgeRail")}
              </span>
              <span className="mt-1 font-fell text-[11px] tracking-[0.14em] text-brass-400">
              ...
              <span className="mt-1 block h-[3px] w-full overflow-hidden rounded-full bg-coal-950">
                <motion.span
                  className="block h-full rounded-full bg-brass-400"
                  animate={{ width: `${roundFrac * 100}%` }}
            ...
            <div className="flex min-w-0 flex-col justify-center leading-tight">
              ...
              >
                <span className="truncate">
                  {mine ? t("game.topbar.yours") : p.name}
                </span>
                  ...
                  {Array.from({ length: maxActions }, (_, i) => (
                    <span
                      ...
                      className={cn(
                        ...
                        i < done
                          ? "border-brass-400 bg-brass-400"
                          : i === done
                            ? "border-brass-400 bg-transparent"
                            : "border-brass-700/60 bg-transparent",
                      )}
                    />
                  ))}
                </span>
        ...
        {mine && stage !== "card" && (
          <div className="relative flex min-w-0 items-center gap-2.5 border-t border-brass-700/40 px-3 py-1.5">
            {verb && (
              <span className="shrink-0 rounded-sm border border-brass-700/70 px-1.5 py-0.5 font-sans text-[9px] font-bold uppercase tracking-[0.16em] text-brass-400">
                {t(VERB_LABEL[verb])}
              </span>
            )}
            {summary && (
              <span
                className="min-w-0 flex-1 truncate font-mono text-[11px] text-cream-100/90"
                title={summary}
              >
                {summary}
              </span>
            ...
            {cost && (
              <span
                className={cn(
                  ...
                  cost.after < 0
                    ? "border-rust-500/70 text-rust-500 brightness-150"
                    : "border-brass-700/60 text-brass-400",
                )}
                title={t("game.hand.costTip")}
              >
                <span>{t("game.hand.total", { n: cost.total })}</span>
                <span className="text-cream-100/40">·</span>
                <span className={cost.after < 0 ? "" : "text-cream-100/75"}>
                  {t("game.hand.left", { n: cost.after })}
                </span>
              </span>
            )}
            {stage === "ready" && (
              <button
                type="button"
                onClick={confirm}
                className="btn-strike !min-h-[32px] !px-4 !py-1 text-xs"
              >
            ...
            {stage === "verb" && (
              <button
                type="button"
                onClick={() => setVerb("pass")}
                className="btn-ledger !min-h-[32px] !px-3 !py-1 text-xs"
                title={t("game.topbar.passTip")}
              >
                {t("game.topbar.pass")}
              </button>
            )}
