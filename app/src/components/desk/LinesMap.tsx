  ...
  return (
    <section aria-label={t('platform.desk.lines.title')}>
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        ...
        <p className="data-text text-[11px] text-iron-400 tnums">{marks.length ? t('platform.desk.lines.legend', { towns: marks.length, tiles, games: lines.games }) : ''}</p>
      </div>
      <div className="gz-rule-double mt-2" aria-hidden />
      <p className="mt-3 font-serif text-[13.5px] italic text-paper-300">{marks.length ? t('platform.desk.lines.lede') : t('platform.desk.lines.none')}</p>
      <div className="gz-engraving mt-4">
        <svg viewBox={`0 0 ${W} ${H}`} className="block w-full" role="img" aria-label={t('platform.desk.lines.title')}>
          ...
          {marks.map(({ town, n }) => {
            ...
            return (
              <g key={town.id}>
                <circle cx={town.x + BLEED_X} cy={town.y + BLEED_Y} r={r} fill="#C9A24B" fillOpacity={0.28} stroke="#8a6b33" strokeWidth={4} />
                <circle cx={town.x + BLEED_X} cy={town.y + BLEED_Y} r={10} fill="#8a6b33" />
                {named.has(town.id) && (
