  ...
  return (
    <section aria-label={t('platform.home.edition.title')}>
      ...
      <div className="mt-1 border-t border-[var(--gz-ink-soft)]">
        {!session ? (
          <p className="px-1 py-5 text-center font-serif text-[13px] text-paper-300">{t('platform.home.edition.signIn')}</p>
        ) : !edition ? (
          <p className="px-1 py-5 text-center font-serif text-[13px] text-paper-300">{t('platform.home.activity.loading')}</p>
        ) : edition.games === 0 ? (
          ...
          <div className="px-1 py-3">
            ...
            <ul className="mt-3 flex flex-col">
              {edition.latest.map((g) => (
                <li key={g.code + g.finishedAt} className="border-b border-[var(--gz-ink-faint)] py-2 font-serif text-[13px] leading-snug text-paper-100 last:border-b-0">
